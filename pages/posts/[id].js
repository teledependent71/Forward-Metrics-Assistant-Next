...
import postsPageInitialPathsBa3f5Resource from '../../resources/posts-page-initial-paths-ba3f5'
import postsPageInitialProps8ae60Resource from '../../resources/posts-page-initial-props-8ae60'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Forward Metrics Assistant</title>
          <meta
            property="og:title"
            content="Posts1 - Forward Metrics Assistant"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsBa3f5Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps8ae60Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
